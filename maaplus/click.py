...
ClickResolver: TypeAlias = Callable[[Rect], Point]
...
def center(rect: Rect) -> Point:
    """Resolve the center point of a rectangular click area."""
    x, y, width, height = _validate_rect(rect)
    return x + width // 2, y + height // 2


def random(padding: float = 0.0) -> ClickResolver:
    """Resolve a random point inside a rectangular click area.
...
def relative(x_ratio: float, y_ratio: float) -> ClickResolver:
    """Resolve a stable relative point within a rectangular click area."""
    if not 0 <= x_ratio <= 1:
