...
from .geometry import PathInterpolator, Point

_Easing: TypeAlias = Callable[[float], float]


def direct(points: Sequence[Point]) -> tuple[Point, ...]:
    """Keep the caller-supplied path unchanged."""
    return tuple(points)
...
def _resample(points: Sequence[Point], samples: int, easing: _Easing) -> tuple[Point, ...]:
    path = tuple(points)
    if len(path) < 2:
        raise ValueError("path interpolation requires at least two points")
